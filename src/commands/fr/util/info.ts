...
export const command: CommandDatas = {
    ...
    execute: async (
        ...
        if(sub === "role") {
            ...
            .setFooter({text: "Alimenté par Aunt Développement", iconURL: bot.djsClient!.user?.avatarURL({extension: "png", forceStatic: false, size: 2048}) ?? undefined})
            ...
            .addFields(
                ...
                {name: `Membres [${role.members.size}/${role.guild.members.cache.size}]`, value: `${role.members.size === 0 ? "Personne ne possède ce rôle :/" : role.members.map((member) => `<@${member.id}>`).slice(0, 9).join(", ")} ${role.members.size > 10 ? `**and more...**` : ""}`, inline: false},
                {name: `Permissions [${role.permissions.toArray().length}/39]`, value: `${role.permissions.toArray().length === 0 ? "Aucune permission incluse avec ce rôle :/" : role.permissions.toArray().map((permission) => permission).slice(0, 9).join(", ")} ${role.permissions.toArray().length > 10 ? "**and more...**" : ""}`, inline: false},
            ...
            .setFooter({text: "Alimenté par Aunt Développement.", iconURL: bot.djsClient!.user?.displayAvatarURL({extension: "png", forceStatic: false, size: 2048}) ?? undefined})
            .setThumbnail(bot.djsClient!.user?.displayAvatarURL() ?? null)
            .setDescription(`${bot.customEmojis.chat} - Informations et statistiques de Aunt Bot.\n**Shard #1 • ${bot.djsClient!.users.cache.size} membres • ${bot.djsClient!.guilds.cache.size} serveurs • ${(await bot.djsClient!.application?.commands.fetch())!.size} commandes**`)
            .addFields(
                {name: "Latence", value: `${bot.djsClient!.ws.ping} millisecondes \`(${Math.round(Date.now() - interaction.createdTimestamp)} millisecondes d'exécution)\`.`, inline: false},
                {name: "Propriétaire", value: `<@${process.env.OWNER}>`, inline: true},
                {name: "Identifiant", value: `${bot.djsClient!.user?.id}`, inline: false},
                {name: "Serveurs", value: `${await getGuildInvites()} ${bot.djsClient!.guilds?.cache.size > 10 ? "**et d'autres...**" : ""}`, inline: false},
                {name: "Création du client", value: `<t:${Math.round(bot.djsClient!.user?.createdTimestamp! / 1000)}>`, inline: true},
                {name: "Connecté depuis", value: `<t:${Math.round(bot.djsClient!.readyTimestamp! / 1000)}>`, inline: true},
            )
