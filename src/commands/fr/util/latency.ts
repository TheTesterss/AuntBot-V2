...
export const command: CommandDatas = {
    ...
    execute: async (
        ...
        let message = await interaction.editReply({
            embeds: [embed.setDescription(`${bot.customEmojis.chat} - Recherche des latences...`)]
        });
        await message.edit({
            embeds: [
                embed.setDescription(`${bot.customEmojis.chat} - Latences trouvées.`).addFields({
                    name: `<t:${Math.round(Date.now() / 1000)}:R>`,
                    value: `${bot.customEmojis.stats} - **MS Passerelle** - \`${bot.djsClient!.ws.ping}ms\`\n${bot.customEmojis.stats} - **MS Réponse** - \`${Math.round(message.createdTimestamp / interaction.createdTimestamp)}ms\`\n${bot.customEmojis.stats} - **Connecté** - <t:${Math.round(bot.djsClient!.readyTimestamp! / 1000)}:R>`,
                    inline: true
                })
            ]
        });
