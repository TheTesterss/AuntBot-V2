    ...
    execute: async (
        ...
        if (sub === "role") {
            ...
            let embed = new EmbedBuilder()
                ...
                .setDescription(
                    `${bot.customEmojis.chat} - Created the  <t:${Math.round(role.createdTimestamp / 1000)}>`
                )
                    ...
                    },
                    {
                        ...
                        value: `${
                            role.permissions.toArray().length === 0
                                ? "No permission are include with this role :/"
                                : role.permissions
                                      .toArray()
            ...
            let embed = new EmbedBuilder()
                ...
                .setDescription(
                    `${bot.customEmojis.chat} - Informations and statistics of the bot.\n**Shard #1 • ${bot.djsClient!.users.cache.size} members • ${bot.djsClient!.guilds.cache.size} guilds • ${(await bot.djsClient!.application?.commands.fetch())!.size} commands**`
                )
                .addFields(
                    {
                        name: "Latency",
                        value: `${bot.djsClient!.ws.ping} millisecondes \`(${Math.round(Date.now() - interaction.createdTimestamp)} millisecondes of executing time)\`.`,
                        inline: false
                    },
